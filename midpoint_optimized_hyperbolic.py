...
midpoint_opt :  Use the midpoint composite rule for computing the integrals 
                in the numerator and denominator in [0,pi]
                 
"""
import numpy as np

def midpoint_opt(time_array, N_fft=32, excen=1.1, epsi=1):
    """    
...
#    
    ...
    mu  = (xmas + xmenos)/2
    rho = (xmas - xmenos)/2
    h   = np.pi/(N_fft-1) # es el paso entre puntos 
    x   = np.linspace(h/2, np.pi-h/2, N_fft-1)[:,np.newaxis]
    ...
    numer = np.sum( tablaN[:],axis=0)
    denom = np.sum( tablaD[:],axis=0)
...
npuntos = 10
N_fft = 32
sol = midpoint_opt(np.linspace(0.1, 20, npuntos), N_fft, excen=2, epsi=1)
print( sol )
